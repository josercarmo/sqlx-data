...
// Test trait with alias definitions using PostgreSQL-specific types and SQL
#[repo]
#[alias(
    user_columns = "id, name, email, age, birth_year"
)]
#[alias(user_table = "users")]
#[alias(count_query = "SELECT COUNT(*) as \"count!: i64\" FROM users")]
#[alias(avg_query = "SELECT AVG(age) FROM users")]
#[alias(where_clause = "WHERE age >= $1")]
trait AliasUserRepo {
    // Basic alias substitution with PostgreSQL parameter style
    ...
    async fn find_users_by_age(&self, min_age: i16) -> Result<Vec<User>>;
    ...
    async fn get_name_email_by_age(&self, min_age: i16) -> Result<Vec<(String, String)>>;

    // Mixed aliases in different parts of query
    #[dml("SELECT name FROM {{user_table}} {{where_clause}}")]
    async fn get_names_by_age(&self, min_age: i16) -> Result<Vec<String>>;
    ...
    // PostgreSQL-specific: Using LIMIT with strong types
    #[dml("SELECT {{user_columns}} FROM {{user_table}} ORDER BY age DESC LIMIT $1")]
    async fn find_oldest_users(&self, limit_count: i64) -> Result<Vec<User>>;

    // PostgreSQL-specific: Using MIN/MAX with proper casting
    #[dml("SELECT MIN(age) as \"min_age!: i16\", MAX(age) as \"max_age!: i16\" FROM {{user_table}}")]
    async fn get_age_range(&self) -> Result<(i16, i16)>;
}
...
mod tests {
    ...
    async fn test_postgresql_specific_limit(pool: Pool) {
        let repo = TestAliasApp { pool };
    ...
    async fn test_postgresql_min_max_types(pool: Pool) {
        let repo = TestAliasApp { pool };
