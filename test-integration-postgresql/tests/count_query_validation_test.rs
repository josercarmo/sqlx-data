...
// User model for count validation tests with PostgreSQL strong types
...
trait CountValidationRepo {
    ...
    #[dml("SELECT COUNT(*) as \"count!: i64\" FROM users WHERE age BETWEEN $1 AND $2")]
    async fn count_users_in_age_range(&self, min_age: i16, max_age: i16) -> Result<i64>;

    // Complex query with LIMIT, OFFSET, GROUP BY, HAVING and parameter order test
    // Uses STRING_AGG (PostgreSQL) instead of GROUP_CONCAT (MySQL/SQLite)
    #[dml(
        r#"
        SELECT age, COUNT(*) as "user_count!: i64", STRING_AGG(name, ',') as "names!: String"
        FROM users
...
mod tests {
    ...
    async fn test_bind_order_consistency(pool: Pool) -> Result<()> {
        ...
        assert!(result.total_items >= 0, "Should return valid count");
        assert_eq!(
