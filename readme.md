...
// Generated by #[repo] and #[dml] macros:
trait UserRepo {
    ...
    async fn find_all_cursor_query(&self, params: impl IntoParams) -> Result<Cursor<User>> {
        let query = sqlx::query_as!(User, "SELECT * FROM users ORDER BY id");
        Cursor::paginate(query, self.get_pool(), params).await
    }

    fn stream_active_query(&self) -> impl Stream<Item = Result<User>> {
        sqlx::query_as::<_, User>("SELECT * FROM users WHERE age >= 18")
            .fetch(self.get_pool())
    }

    // Original methods now have default implementations calling _query methods
    async fn find_by_id(&self, id: i64) -> Result<User> {
    ...
    async fn find_all_cursor(&self, params: impl IntoParams) -> Result<Cursor<User>> {
        self.find_all_cursor_query(params).await
    }

    fn stream_active(&self) -> impl Stream<Item = Result<User>> {
        self.stream_active_query()
    }
