...
mod tests {
    ...
    async fn test_cursor_has_prev_scenarios(pool: Pool) {
        let repo = TestUserRepo { pool: &pool };

        // Test 1: Without providing cursor params - should use fallback with has_prev = false
        let no_cursor_params = ParamsBuilder::new()
            .sort()
                .asc("id")
                .done()
            .cursor()
        ...
        let no_cursor_page = repo.find_all(no_cursor_params).await.unwrap();
        println!(
            "No cursor: has_prev={}, has_next={}, data_len={}",
            no_cursor_page.has_prev,
            no_cursor_page.has_next,
            no_cursor_page.data.len()
        );

        // This should use fallback behavior where has_prev = false
        assert!(
            !no_cursor_page.has_prev,
            "No cursor should not have previous"
        );
        // First page should have next cursor available if there's more data

        // Test 2: Cursor pointing to beginning of data (before first item)
        // Using before with the smallest possible value
        let beginning_params = ParamsBuilder::new()
            .sort()
            .asc("id")
            .done()
            .cursor()
            .before(1) // Before first user (id=1)
            .done()
            .limit(3)
            .build();

        let beginning_page = repo.find_all(beginning_params).await.unwrap();
        println!(
            "Before first item: has_prev={}, has_next={}, data_len={}",
            beginning_page.has_prev,
            beginning_page.has_next,
            beginning_page.data.len()
        );

        // This should return empty results since there's nothing before id=1
        assert_eq!(
            beginning_page.data.len(),
            0,
            "Before first item should return empty"
        );
        // When cursor points before first item, has_prev = false (no previous data exists)
        assert!(
            !beginning_page.has_prev,
            "Before first item should have has_prev=false (no previous data)"
        );

        // Test 3: Compare with after at beginning
        let after_beginning_params = ParamsBuilder::new()
            .sort()
            .asc("id")
            .done()
            .cursor()
            .after(0) // After id=0 (before first user)
            .done()
            .limit(3)
            .build();

        let after_beginning_page = repo.find_all(after_beginning_params).await.unwrap();
        println!(
            "After id=0: has_prev={}, has_next={}, data_len={}",
            after_beginning_page.has_prev,
            after_beginning_page.has_next,
            after_beginning_page.data.len()
        );

        // This should have data (users 1,2,3)
        assert_eq!(after_beginning_page.data.len(), 3);
        assert!(
            after_beginning_page.has_prev,
            "After id=0 cursor should have previous"
        );
    }
    ...
    async fn test_cursor_edge_cases(pool: Pool) {
        ...
        // Test with limit = 1 (smallest page size)
        let tiny_page_params = ParamsBuilder::new()
            .sort()
            .asc("id")
            .done()
            .cursor()
        ...
        let tiny_page = repo.find_all(tiny_page_params).await.unwrap();
        println!(
            "Tiny page: has_next={}, has_prev={}, data_len={}",
            tiny_page.has_next,
            tiny_page.has_prev,
            tiny_page.data.len()
        );
        ...
        assert!(
            tiny_page.has_next,
            "Tiny page should have next (19 more users)"
        );
        assert!(
            tiny_page.has_prev,
            "Tiny page with cursor has previous (cursor-based pagination)"
        );
        assert_eq!(tiny_page.data.len(), 1);

        // Test exact boundary - request exactly remaining items
    ...
    async fn test_mixed_sort_directions_should_work(pool: Pool) {
        let repo = TestUserRepo { pool: &pool };

        // Mixed ASC and DESC directions should now work with OR-based cursor pagination
        let params = ParamsBuilder::new()
            .sort()
                .asc("name")
                .desc("id")  // Mixed direction should now work
                .done()
            .cursor()
                .first_page()
                .done()
    ...
    async fn test_all_asc_should_work(pool: Pool) {
        let repo = TestUserRepo { pool: &pool };
        ...
        let result = repo.find_all(params).await;
        assert!(result.is_ok(), "All ASC should work: {}", result.err().unwrap());
    }
    ...
    async fn test_all_desc_should_work(pool: Pool) {
        let repo = TestUserRepo { pool: &pool };
        ...
        let result = repo.find_all(params).await;
        assert!(result.is_ok(), "All DESC should work: {}", result.err().unwrap());
    }
}
