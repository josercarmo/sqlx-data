...
use sqlx_data::database::{Connection, Executor, Pool, QueryResult, Result, Transaction};
use sqlx_data::filters::{CursorSecureExtract, CursorValue, FilterValue};
use sqlx_data::pagination::{Cursor, Serial, Slice};
use sqlx_data::params::IntoParams;
use sqlx_data::{CursorData, dml, repo}; // macros ficam no root
...
trait UserPoolRepo {
    ...
    async fn get_user_info_with_pool(&self, id: i64, pool: &Pool) -> Result<(String, i64)>;
    ...
    #[dml("SELECT id, name, email, age FROM users WHERE id = $1")]
    async fn find_by_id_with_transaction(&self, id: i64, tx: &mut Transaction<'_>) -> Result<User>;

    // Serial pagination with Connection and Transaction
    #[dml("SELECT id, name, email, age FROM users")]
    async fn find_users_serial_with_conn(
        &self,
    ...
    #[dml("SELECT id, name, email, age FROM users")]
    async fn find_users_serial_with_tx(
        &self,
        params: impl IntoParams,
    ...
    #[dml("SELECT id, name, email, age FROM users")]
    async fn find_users_slice_with_conn(
        &self,
        params: impl IntoParams,
        conn: &mut Connection,
    ) -> Result<Slice<User>>;
    ...
    async fn insert_with_executor<'e, E: Executor<'e>>(
        &self,
        executor: E,
        value: i64,
    ) -> Result<QueryResult>;
    ...
    async fn insert_complex_executor<'e, EX: Executor<'e>>(
        &self,
        executor: EX,
        name: String,
    ) -> Result<QueryResult>
...
mod tests {
    use super::*;
    use sqlx_data::params::ParamsBuilder;
    use sqlx_data::params::SerialBuilder;
    ...
    async fn test_serial_pagination_with_connection() {
        ...
        let mut conn = pool.acquire().await.unwrap();
        let params = SerialBuilder::new().page(1, 2);
        let result = app
            .find_users_serial_with_conn(params, &mut conn)
            .await
            .unwrap();
    ...
    async fn test_serial_pagination_with_transaction() {
        ...
        let params = SerialBuilder::new().page(1, 2);
        let result = app
            .find_users_serial_with_tx(params, &mut tx)
            .await
            .unwrap();
    ...
    //         .done()
    //         .build();
    //     let result = app.find_users_cursor_with_transaction(params, &mut tx).await.unwrap();
    ...
    //     let result = app.find_users_slice_with_connection(params, &mut conn).await.unwrap();
    ...
    //     let mut tx = pool.begin().await.unwrap();
    //     let params = SliceParams::new(1, 2);
    //     let result = app.find_users_slice_with_transaction(params, &mut tx).await.unwrap();
