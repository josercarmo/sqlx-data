...
trait NamedParamRepo {
    ...
    async fn find_by_email_and_age(&self, age: u8, email: String)
    -> Result<Vec<User>, sqlx::Error>;

    // Mixed with positional (should still work for existing code)
    #[dml("SELECT id, name, email, age as 'age: u8' FROM users WHERE id = $1")]
    async fn find_by_id_positional(&self, id: i64) -> Result<Option<User>, sqlx::Error>;

    // Repeated named parameter
    #[dml(
        "SELECT id, name, email, age as 'age: u8' FROM users WHERE (name = @name OR email = @name) AND age > @min_age"
    )]
