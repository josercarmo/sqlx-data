...
// This is what the user writes with combined approach for MySQL
#[sqlx_data::repo]
trait UserRepo {
    #[dml("SELECT 1")]
    ...
    #[dml(
        "SELECT id as 'id!: Id', name, email, age as 'age: u8', birth_year as 'birth_year: u16' FROM users WHERE id = ?"
    )]
    ...
    // MySQL-specific queries with strong types
    #[dml("SELECT * FROM users WHERE age >= ? ORDER BY age ASC")]
    ...
    #[dml("SELECT CAST(COUNT(*) AS UNSIGNED) as 'count!: u64' FROM users WHERE age >= ?")]
    async fn count_adults(&self, min_age: u8) -> Result<u64>;

    // Test nullable fields with MySQL types
    #[dml("SELECT name FROM users WHERE birth_year IS NULL")]
    async fn get_names_without_birth_year(&self) -> Result<Vec<String>>;

    // MySQL-specific: Using LIMIT with strong typing
    #[dml("SELECT * FROM users ORDER BY age DESC LIMIT ?")]
    async fn get_oldest_users(&self, limit_count: u32) -> Result<Vec<User>>;

    // MySQL unsigned integer aggregations
    #[dml("SELECT AVG(age) as avg_age FROM users")]
    async fn get_average_age(&self) -> Result<Option<BigDecimal>>;

    #[dml("SELECT MIN(age) as 'min_age!: u8', MAX(age) as 'max_age!: u8' FROM users WHERE age > 0")]
    async fn get_age_bounds(&self) -> Result<(u8, u8)>;

    // Insert with MySQL AUTO_INCREMENT
    #[dml("INSERT INTO users (name, email, age, birth_year) VALUES (?, ?, ?, ?)")]
    async fn create_user(&self, name: String, email: String, age: u8, birth_year: Option<u16>) -> Result<QueryResult>;

    // Update with MySQL syntax
    #[dml("UPDATE users SET age = ? WHERE id = ?")]
    async fn update_user_age(&self, age: u8, id: i64) -> Result<QueryResult>;
