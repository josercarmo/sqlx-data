...
trait TimeRepo {
    ...
    #[dml("SELECT id, name, created_date, created_time, created_datetime, created_offset FROM time_records WHERE id = ?")]
    async fn find_by_id(&self, id: i64) -> Result<Option<TimeRecord>>;

    #[dml("SELECT id, name, created_date, created_time, created_datetime, created_offset FROM time_records WHERE created_date = ?")]
    async fn find_by_date(&self, date: Date) -> Result<Vec<TimeRecord>>;

    #[dml("SELECT id, name, created_date, created_time, created_datetime, created_offset FROM time_records WHERE created_time >= ?")]
    async fn find_by_time_after(&self, time: Time) -> Result<Vec<TimeRecord>>;

    #[dml("SELECT id, name, created_date, created_time, created_datetime, created_offset FROM time_records WHERE created_datetime BETWEEN ? AND ?")]
    ...
    #[dml("SELECT id, name, created_date, created_time, created_datetime, created_offset FROM time_records WHERE created_offset > ?")]
...
mod tests {
    ...
    async fn test_insert_and_retrieve_time_types(pool: Pool) {
        ...
        let record = app.find_by_id(result.last_insert_id() as i64).await.unwrap();
        ...
        assert_eq!(record.created_offset, test_offset);
    }
    ...
    async fn test_query_by_date(pool: Pool) {
        ...
        assert!(records.iter().any(|r| r.id == result.last_insert_id() as i64));
    }
    ...
    async fn test_current_timestamp_functions(pool: Pool) {
        ...
        let record = record.unwrap();
        assert_eq!(record.created_date, current_date);
    }
}