...
trait UncheckedRepo {
    ...
    #[dml(
        "SELECT id, name, email, age, birth_year FROM users WHERE id = ?",
        unchecked
    )]
    async fn find_user_by_id(&self, id: i64) -> Result<User>;

    // Test unchecked tuple queries
    #[dml("SELECT id, name, email FROM users WHERE age > ?", unchecked)]
    async fn find_user_tuples(&self, min_age: u8) -> Result<Vec<(i64, String, String)>>;

    // Test unchecked optional queries
    #[dml("SELECT id, name FROM users WHERE email = ?", unchecked)]
    async fn find_user_optional(&self, email: String) -> Result<Option<(i64, String)>>;

    // Test unchecked queries returning Vec<struct>
    #[dml(
        "SELECT id, name, email, age, birth_year FROM users WHERE age BETWEEN ? AND ?",
        unchecked
    )]
    async fn find_users_in_age_range(&self, min_age: u8, max_age: u8) -> Result<Vec<User>>;
    ...
    #[dml(
        "INSERT INTO users (name, email, age, birth_year) VALUES (?, ?, ?, ?)",
        unchecked
    )]
    ...
    #[dml("DELETE FROM users WHERE id = ?", unchecked)]
    async fn delete_user(&self, id: i64) -> Result<()>;

    // Test unchecked queries with complex SQL that might not parse well
    #[dml(
        "SELECT id, name, email, age, (age * 365) as approximate_days FROM users WHERE birth_year > ?",
        unchecked
    )]
    async fn get_users_with_calculated_field(
        &self,
        min_birth_year: u16,
    ) -> Result<Vec<(i64, String, String, u8, u16)>>;
...
mod tests {
    ...
    async fn test_complex_calculated_field_with_unchecked(pool: Pool) {
        ...
        for (id, name, email, age, approx_days) in &users {
            assert!(*age > 0);
            assert_eq!(*approx_days, (*age as u16) * 365);
