...
trait UserStreamRepo {
    ...
    fn stream_all_users(&self) -> impl Stream<Item = Result<User>>;

    // Stream with WHERE clause
    #[dml("SELECT id, name, email, age, birth_year FROM users WHERE age >= ?")]
    fn stream_users_by_min_age(&self, min_age: u8) -> impl Stream<Item = Result<User>> + Send;
    ...
    // Stream with LIMIT
    #[dml("SELECT id, name, email, age, birth_year FROM users LIMIT ?")]
    fn stream_users_limited(&self, limit: u32) -> impl Stream<Item = Result<User>> + Send;

    // Stream tuple results
    #[dml("SELECT name, age FROM users WHERE age BETWEEN ? AND ?")]
    fn stream_user_info(&self, min_age: u8, max_age: u8) -> impl Stream<Item = Result<(String, u8)>> + Send;

    // Stream with complex WHERE
    #[dml("SELECT id, name, email, age, birth_year FROM users WHERE age >= ? AND name LIKE ?")]
    fn stream_users_by_age_and_name(&self, min_age: u8, name_pattern: String) -> impl Stream<Item = Result<User>> + Send;
    ...
    // Stream with multiple joins (if we had related tables)
    #[dml("SELECT DISTINCT name FROM users WHERE age IN (SELECT age FROM users WHERE age > ?)")]
    fn stream_names_by_subquery(&self, min_age: u8) -> impl Stream<Item = Result<String>>;
}
...
mod tests {
    ...
    async fn test_stream_filter_map(pool: Pool) {
        ...
        assert!(adult_names.len() > 0);
        // Verify that we only get names from adults
        // We can't easily verify ages here since we only collected names
    ...
    async fn test_stream_fold(pool: Pool) {
        ...
        // Use manual loop to calculate total age
        while let Some(user_result) = stream.next().await {
            let user = user_result.unwrap();
            total_age += user.age as u32;
        }
    ...
    async fn test_stream_for_each(pool: Pool) {
        ...
        // Use manual loop to process each user
        while let Some(user_result) = stream.next().await {
            let user = user_result.unwrap();
            processed_count += 1;
